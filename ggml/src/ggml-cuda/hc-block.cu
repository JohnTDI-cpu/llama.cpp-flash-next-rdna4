...
// ---- A0: grid = nt*hc (blok = jeden strumien jednego tokena), blok 1024
...
hcblock_a0_f32(const float * __restrict__ x, const float * __restrict__ w_norm, float * __restrict__ xn_out,
    ...
    const int t   = blockIdx.x / hc;
    const int c   = blockIdx.x - t * hc;
    ...
    const float * wr = w_norm + (int64_t) c * n_embd;
    float * xo = xn_out + (int64_t) t * hc_dim + (int64_t) c * n_embd;
    for (int col = tid; col < n_embd; col += HCB_NORM_THREADS) { xo[col] = hcb_xn_elem(xr[col], scale, wr[col]); }
    // kwantyzacja: bloki q8_1 tego strumienia (n_embd % 32 == 0 -> granice blokow pokrywaja sie ze strumieniami)
    const int warp = tid / WARP_SIZE, lane = tid % WARP_SIZE;
    const int b0 = (c * n_embd) / QK8_1, nb = n_embd / QK8_1;
    for (int b = warp; b < nb; b += HCB_NORM_THREADS / WARP_SIZE) {
        const int col = b * QK8_1 + lane;
        const float xi = hcb_xn_elem(xr[col], scale, wr[col]);
        hcb_quantize_block(xi, &yq[t * nblk + b0 + b], lane);
    }
}
...
#pragma unroll
                for (int i = 0; i < rows; ++i) {
                    if (vlane == i) {
                        ...
                        gated_s[j][c][e0 + i] = johnv8_mul_nc(xn[(int64_t) j * hc_dim + row0 + i], sg);
                    }
                }
            }
        }
...
bool ggml_cuda_hcblock_a_ok(const ggml_cgraph * cgraph, int node_idx) {
    ...
    if (x->ne[3] != 1 || nt < 1 || nt > HCB_MAX_NT || hc != 4) { HCB_REJECT("a", 5); }
...
bool ggml_cuda_hcblock_b_ok(const ggml_cgraph * cgraph, int node_idx, int * n_extra) {
    ...
    if (nt < 1 || nt > HCB_MAX_NT || hc != 4 || n_embd % HCB_B_ELEMS != 0 || R % 32 != 0) { HCB_REJECT("b", 7); }
